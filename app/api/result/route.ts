...
export async function POST(req: NextRequest) {
  ...
  const { rows: maxRows } = await db.query('SELECT ISNULL(MAX(result_id),0)+1 AS next_id FROM quiz_result');
  const resultId = maxRows[0].next_id;

  await db.query(
    `INSERT INTO quiz_result (result_id, user_id, mode, lv, score, total_questions, correct_count, time_sec, played_at)
