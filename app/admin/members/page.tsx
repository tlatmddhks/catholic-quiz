...
export default async function MembersPage({ searchParams }: Props) {
  ...
  const [listRes, countRes] = await Promise.all([
    db.query(
      `SELECT u.user_id, u.name, u.nickname, u.christen, u.parish, u.church, u.email, u.created_at,
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
              {['이름','세례명','본당/성당','이메일','플레이수','가입일'].map(h => (
          ...
          <tbody>
            {members.length === 0 ? (
              <tr><td colSpan={6} style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>회원이 없습니다</td></tr>
            ) : members.map((m: any) => (
